...
class Colake(Base_Connector):

    def __init__(self, args):
        super().__init__()

        if args.colake_model_dir is not None:
            # load bert model from file
            colake_model_name = os.path.join(args.data_path, args.colake_model_dir)
            dict_file = colake_model_name
            print("loading Colake model from {}".format(colake_model_name))
        else:
            print("Can't continue because Colake model path is missing!")

        # When using a cased model, make sure to pass do_lower_case=False directly to BaseTokenizer
        do_lower_case = False
        if 'uncased' in colake_model_name:
            do_lower_case = True
        ...
        states_dict = torch.load(dict_file)

        self.masked_colake_model = RobertaForMaskedLM(config=config)
        self.masked_colake_model.load_state_dict(states_dict, strict=False)
        self.masked_colake_model.eval()
        # print(self.masked_roberta_model.config)

        # ... to get hidden states
        self.colake_model = RobertaModel(config=config)
        self.colake_model.load_state_dict(states_dict, strict=False)
        self.colake_model.eval()
    ...
    def get_batch_generation(self, sentences_list, logger=None, try_cuda=True):
        ...
        with torch.no_grad():
            logits = self.masked_colake_model(
